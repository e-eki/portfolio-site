...
function showPage(event) {
    ...
    if (content.classList.contains('page__content_shown')) return; 

    // первый скролл (зачастую) автоматический, поэтому ничего не делаем
    ...
    matrixBackground.classList.remove('matrix-background_clickable');
    content.classList.remove('content_hidden');
    content.classList.add('content_shown');
...
function addPageFunctionality() {
    ...
    var aboutSection = document.querySelector('.js-section-about');
    var summarySection = document.querySelector('.js-section-summary');
    var worksSection = document.querySelector('.js-section-works');
    var contactsSection = document.querySelector('.js-section-contacts');
    ...
    aboutItem.addEventListener("click", showActiveSection);
    summaryItem.addEventListener("click", showActiveSection);
    worksItem.addEventListener("click", showActiveSection);
    contactsItem.addEventListener("click", showActiveSection);

    function showActiveSection(event) {
        ...
        elements.forEach(function(element) {
            ...
            element.section.classList.remove('section_active');
            element.section.classList.add('section_hidden');

            if (element.item == event.target.parentNode) {
                element.item.classList.add('menu__item_active');
                element.section.classList.remove('section_hidden');
                element.section.classList.add('section_active');
                element.item.scrollIntoView();

                for (var i = 0; i < element.section.childNodes.length; i++) {
                   
                    var elem = element.section.childNodes[i];

                    if (elem.classList && elem.classList.contains("section__heading") &&
                        !elem.classList.contains("section__heading_shown")) {
                            setDrawHeaderTimer(elem); 
                    }

                    if (elem.classList && elem.classList.contains("section__description")
                        && elem.classList.contains("section__description_hidden")) {
                            elem.classList.remove("section__description_hidden");
                            elem.classList.add("section__description_shown");
                    }
                }
            }
        }) 
    ...
    function setDrawHeaderTimer(elem) {                                  
        ...
        function drawHeader() {
            elem.classList.add('section__heading_shown');

            if (counter > header.length) {
                elem.innerHTML = header.slice(0, counter) + '<span class="section__heading_blinking">_</span>';
                cancelAnimationFrame(drawHeaderRequest);
            }
            else {
                //elem.innerHTML = header.slice(0, counter) + '<span class="section__heading_blinking">_</span>';
                elem.innerHTML = header.slice(0, counter) + '_';
                counter++;
...
function fillMatrix() {
    ...
    matrixBackground.width = document.documentElement.clientWidth;
    var fontSize = 10;  //???  
    ...
    for (var x = 0; x < columns; x++) {
        ...
        column.className = 'matrix-background__column';
        ...
        for (var y = 0; y < rows; y++) {
            ...
            letter.className = 'matrix-background__letter';
            letter.classList.add('matrix-background__letter_color-main');
            letter.x = x;
            letter.y = y;
...
function fillMatrix() {
    ...
    for (var x = 0; x < columns; x++) {
        ...
        // рандомная координата У или null для каждого столбца - для текущего подсвеченного символа в столбце
        // чтобы не по всем столбцам сбегали символы - выбираем некоторые по Math.random() > 0.5
        counters[x] = Math.random() > 0.5 ? Math.floor(Math.random()*rows) : null;
        ...
        for (var i = 0; i < columns; i++) {
            ...
            for (var j = drops[i].length-1; j >= 0; j--) {
                ...
                // во всех остальных случаях прозрачность уменьшается на opacityUnit
                    ...
                    // если мы не на стартовой странице, или символ попадает в след мыши, то при достижении прозрачности 0.5
                    ...
                    if (!isStartPage && drops[i][j].mouseMove == true && newOpacity <= 0.5) {
                        ...
                        drops[i][j].classList.remove('matrix-background__letter_color-lighter');
                        //drops[i][j].classList.remove('matrix-background__letter_color-main');
                        drops[i][j].classList.add('matrix-background__letter_color-main');
                    }
                        
                }
                // когда прозрачность становится = 0, т.е. символ исчезает, убираем флаг mouseMove  
                if (newOpacity <= 0) {
                    //drops[i][j].style.color = '#0F0';
                    drops[i][j].classList.remove('matrix-background__letter_color-lighter');
                        //drops[i][j].classList.remove('matrix-background__letter_color-main');
        ...
        for (var j = event.target.y - 15; j < event.target.y + 15; j++) {
            ...
            if (drops[i] && drops[i][j] && drops[i][j].style.opacity > 0 &&
                ...
                drops[i][j].mouseMove == false && Math.random() > 0.45) {
                    ...
                    drops[i][j].classList.remove('matrix-background__letter_color-main');
                    drops[i][j].classList.remove('matrix-background__letter_color-lighter');
                    drops[i][j].classList.add('matrix-background__letter_color-lighter');
...
function drawMouseTrackForHomePage(event) {
    // ширина и высота следа мыши
    // если мышь не над матрицей, а над блоком контента, то кооридинату определяем с помощью event.clientY,
    // она будет менее точна, чем event.target.y, но для этого режима сойдет
...
function drawMatrix1() {
    ...
    function drawMouseTrackForHomePage(event) {
        ...
        for (var i = 0; i < columns; i++) {
            for (var j = currentRow - 5; j < currentRow + 5; j++) {
                if (drops[i] && drops[i][j] && Math.random() > 0.975) {
...
//------------------------------СОВСЕМ СТАРОЕ------------------------------------------
//попытка вручную реализовать скролл по всем разделам страницы (все видны сразу)
